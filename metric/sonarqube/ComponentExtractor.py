...
def get_findings_count_by(group, regex, sev, excel_input):
    ...
    for i in excel_input.index:
        ...
        group_name = excel_input['componentName'][i]
        severity = excel_input['severity'][i]
        status = excel_input['status'][i]
        if regex in field_name and group in group_name and sev == severity and status == "OPEN":
            count += 1
    return count;
...
if file_name.endswith("xlsx"):
    ...
    excel_input = pandas.ExcelFile(file_name)._parse_xls(sheet_name)
