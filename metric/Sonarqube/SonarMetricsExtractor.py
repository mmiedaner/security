...
def parse_commandline_paramters():
    ...
    parser.add_argument('--creds', metavar='creds', help='path to credentials file')
...
api_client = SonarAPIClient(main_url, configuration)
...
if analyze:
    ...
    if len(df4) != 0 and analyze:
        ...
        if configuration:
            ...
            # Removed CVSS filter to see all Wontfix CWEs
            ...
            cwe_injection = data_analyser.analyse_by_applications(df_wontfix, configuration['applications'], injection_filter)
            cwe_iam = data_analyser.analyse_by_applications(df_wontfix, configuration['applications'], iam_filter)
    ...
    if df2 is not None and len(df2) != 0 and df_wontfix_analysis is not None and len(df_wontfix_analysis) != 0:
        frames.append(pandas.concat([df2, df_separator, df_wontfix_analysis], axis=1))
    elif df2 is not None and len(df2) != 0 and df_wontfix_analysis is not None and len(df_wontfix_analysis) == 0:
        frames.append(df2)
    elif df2 is not None and len(df2) == 0 and df_wontfix_analysis is not None and len(df_wontfix_analysis) != 0:
    ...
    if df_wontfix_analysis is not None and len(df_wontfix_applications) != 0:
